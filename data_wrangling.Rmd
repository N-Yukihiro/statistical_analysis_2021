...
毎回利用するパッケージを読み込む
パッケージのインストールがされていない場合はインストールを先に行う
...
`tidylog`パッケージは必ずしも必要ないが、初心者は使っておくと何をしているのかわかって便利

## 利用するデータ

まず, R内にデフォルトで格納されている iris[^1][^2]  (アヤメの花のデータ) を使う

```{r}
head(iris)
```

[^1]: Anderson, Edgar. 1936. “The Species Problem in Iris.” Annals of the Missouri Botanical Garden 23(3): 457.

[^2]: FISHER, R. A. 1936. “THE USE OF MULTIPLE MEASUREMENTS IN TAXONOMIC PROBLEMS.” Annals of Eugenics 7(2): 179–88.
...
横型データを縦型データにしたり, その逆にしたりするには, `tidyr::pivot_longer()`, `tidyr::pivot_wider()`を利用する[^3].

コンピューター言語は基本的に縦型データのほうが扱いやすい
R界隈では特にtidy dataという

[^3]: なぜこのような変換が必要なのかはtidy dataという考え方がポイントになります.詳しくは, 例えば [整然データとは何か](http://id.fnshr.info/2017/01/09/tidy-data-intro/) などを参照してください.
...
データセットとデータセットを結合するには, `*_join`関数群 [^4]を使う
...
[^4]: `*`は正規表現を意味し, `_join`関数は`full_join`や`left_join`などのように`_join`が共通する関数を指す
